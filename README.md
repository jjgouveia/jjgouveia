...
- **Page Comparison Tool** — especificação visual de mudanças em telas: pins numerados sobre imagens antes/depois, revisão por papel (Master/Manager/Employee), chat por tela com WebSocket e notificações. Também abriga o **Tasker**, MCP server HTTP (8 tools) que expõe as tarefas para agentes de IA de qualquer repo, com autenticação por PAT, hash SHA-256 e botões de instalação de 1 clique para Cursor e Claude Code.
